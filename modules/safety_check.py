def check_safety(student_profile, advice_draft):
    ...
    # --- LAYER 1: THE BURNOUT CIRCUIT BREAKER ---
    ...
    for flag in critical_states:
        if flag in state:
            logs.append(f"⚠️ CRITICAL STATE DETECTED: '{flag}'. Activating Mental Health Protocol.")
            safe_reply = (
    ...
    # --- LAYER 2: TOXIC POSITIVITY & FALSE PROMISES FILTER ---
    ...
    for phrase in unsafe_phrases:
        if phrase in advice_draft.lower():
            logs.append(f"⚠️ SAFETY VIOLATION: Found banned phrase '{phrase}'. Rewriting...")
